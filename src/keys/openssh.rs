...
pub struct OpenSSH {
    cipher: HashMap<String, Box<dyn Decrypt>>,
}
...
impl super::KeyParser for OpenSSH {
    ...
    fn parse_privatekey(&mut self, binary: &[u8], passphrase: Option<&[u8]>) -> Result<(String, Vec<u8>, Vec<u8>)> {
        let content = std::str::from_utf8(binary).map_err(|_| Error::invalid_format("not uft-8 string"))?;
        ...
        match passphrase {
            Some(passphrase) => {
                if cipher != b"none" {
                    ...
                    let cipher = self.cipher.get_mut(String::from_utf8(cipher)?.as_str())
